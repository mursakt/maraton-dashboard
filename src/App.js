...
// Forma score: HRV (40%), spanje (35%), stres (25%) → 1-10
function izracunajFormo(hrv, spanje, stres) {
  let score = 0; let factors = 0
  if (hrv) {
    // HRV: <30=1, 30-40=3, 40-50=5, 50-60=7, 60-70=8, 70+=10
    const hScore = hrv < 30 ? 1 : hrv < 40 ? 3 : hrv < 50 ? 5 : hrv < 60 ? 7 : hrv < 70 ? 8 : 10
    score += hScore * 0.4; factors += 0.4
  }
  if (spanje) {
    // Spanje: <5=1, 5-6=3, 6-6.5=5, 6.5-7=6, 7-7.5=7.5, 7.5-8=9, 8+=10
    const sScore = spanje < 5 ? 1 : spanje < 6 ? 3 : spanje < 6.5 ? 5 : spanje < 7 ? 6 : spanje < 7.5 ? 7.5 : spanje < 8 ? 9 : 10
    score += sScore * 0.35; factors += 0.35
  }
  if (stres) {
    // Stres: >75=1, 75-60=3, 60-45=5, 45-35=6, 35-25=8, <25=10
    const stScore = stres > 75 ? 1 : stres > 60 ? 3 : stres > 45 ? 5 : stres > 35 ? 6 : stres > 25 ? 8 : 10
    score += stScore * 0.25; factors += 0.25
  }
  if (factors === 0) return null
  return Math.round((score / factors) * 10) / 10
}

function formaColor(score) {
...
function formaLabel(score) {
  if (!score) return '—'
  if (score >= 8) return 'Odlično'
  if (score >= 6) return 'Dobro'
  if (score >= 4) return 'Povprečno'
  if (score >= 2) return 'Slabo'
  return 'Kritično'
}
...
const css = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  @media(max-width:768px){.grid2,.grid3,.grid4,.grid5{grid-template-columns:1fr}.app{padding:16px}.tabs{flex-wrap:wrap}}
`
...
export default function App() {
  ...
  return (
    <div className="app"><style>{css}</style>
      ...
      {tab === 'pregled' && <TabPregled workouts={workouts} metrike={metrike} currentTeden={currentTeden} formaScore={formaScore} />}
...
function TabPregled({ workouts, metrike, currentTeden, formaScore }) {
  const planTeden = PLAN.find(p => p.teden === currentTeden)
  ...
  return (<>
    ...
    <div className="grid3">
      ...
      <StatCard title="Spanje (zadnje)" value={z.spanje_h ? fmt(z.spanje_h) : '—'} unit="h" sub="cilj: >7.5h" color={z.spanje_h >= 7.5 ? '#22c55e' : z.spanje_h >= 6.5 ? '#eab308' : '#ef4444'} />
      <StatCard title="Stres (zadnji)" value={z.stres_povprecje ? fmt(z.stres_povprecje, 0) : '—'} sub="cilj: <30" color={!z.stres_povprecje ? '#6b7280' : z.stres_povprecje < 30 ? '#22c55e' : z.stres_povprecje < 50 ? '#eab308' : '#ef4444'} />
    </div>
    <div className="card">
      ...
      <div className="workout-list">
        {workouts.slice(0, 5).map((w, i) => (
          <div key={i} className="workout-item">
            ...
            <span className="type">{w.naziv || w.tip_treninga || '—'}</span>
            <span className="detail">{fmt(w.razdalja_km)} km · {w.povprecni_tempo || '—'}/km · {fmt(w.trajanje_min, 0)} min</span>
            <span className="hr-badge" style={{ background: hrZonaColor(w.povprecni_hr) + '22', color: hrZonaColor(w.povprecni_hr) }}>{w.povprecni_hr ? `${w.povprecni_hr} bpm` : '—'}</span>
          </div>
        ))}
    ...
    <div className="card">
      ...
      <div className="workout-list" style={{ maxHeight: 420, overflowY: 'auto' }}>
        {workouts.map((w, i) => (
          <div key={i} className="workout-item">
            <span className="date">{w.datum?.slice(5)}</span>
            <span className="type" style={{ fontSize: 11, minWidth: 80 }}>{w.naziv || w.tip_treninga || '—'}</span>
            <span className="detail">{w.razdalja_km > 0 ? `${fmt(w.razdalja_km)} km · ` : ''}{w.povprecni_tempo ? `${w.povprecni_tempo}/km · ` : ''}{fmt(w.trajanje_min, 0)} min</span>
            {w.vo2max > 0 && <span style={{ fontSize: 11, color: '#a78bfa', fontFamily: 'DM Mono' }}>VO2: {fmt(w.vo2max, 1)}</span>}
            <span className="hr-badge" style={{ background: hrZonaColor(w.povprecni_hr) + '22', color: hrZonaColor(w.povprecni_hr) }}>{w.povprecni_hr || '—'} bpm</span>
          </div>
        ))}
        {workouts.length === 0 && <div className="empty">Ni podatkov</div>}
      </div>
...
function TabTelo({ metrike }) {
  // Teža: dejanska vs plan
  const tezaDejansko = metrike.filter(m => m.teza_kg && m.datum >= '2026-04-20').slice(0, 60).reverse()
  const tezaGraf = tezaDejansko.map(m => {
    const planZaTa = PLAN.slice().reverse().find(p => p.datum <= m.datum)
    return { datum: m.datum?.slice(5), dejanska: m.teza_kg, plan: planZaTa?.ciljnaKg || null }
  })
  ...
  // Forma trend zadnjih 14 dni
  const formaData = metrike.slice(0, 14).reverse().map(m => ({
    datum: m.datum?.slice(5),
    forma: izracunajFormo(m.hrv, m.spanje_h, m.stres_povprecje)
  })).filter(d => d.forma !== null)
