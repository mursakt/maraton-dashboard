...
export function AlarmiPanel({ workouts, metrike, prehrana, predikcija, compact = false }) {
  ...
  // ── PREDIKCIJA VS CILJ 3:45 ───────────────────────────────
  if (predikcija?.casFinal) {
    const cilj = 13500 // 3:45:00 v sekundah
    const pred = predikcija.casFinal
    const predH = Math.floor(pred / 3600)
    const predM = Math.floor((pred % 3600) / 60)
    const predS = Math.floor(pred % 60)
    const predStr = `${predH}:${String(predM).padStart(2, '0')}:${String(predS).padStart(2, '0')}`
    const razlikaMin = Math.round((pred - cilj) / 60)
    if (razlikaMin > 20)
      alarmi.push({ nivo: 'kritično', kat: 'Forma', naziv: `Napoved maratona ${predStr} — ${razlikaMin} min nad ciljem 3:45`, fix: 'Poveča tedenski obseg in dodaj 1–2 intervalna treninga (tempo, fartlek) na teden.' })
    else if (razlikaMin > 0)
      alarmi.push({ nivo: 'opozorilo', kat: 'Forma', naziv: `Napoved maratona ${predStr} — ${razlikaMin} min nad ciljem 3:45`, fix: 'Vzdrži obseg in intenzivnost — trend gre v pravo smer.' })
  }
  ...
  const danVTednu = now.getDay()
  if (planTeden && planTeden.km > 0 && (danVTednu >= 4 || danVTednu === 0)) {
  ...
  // TEŽA vs plan cilj
  if (tezaNow && planTeden?.ciljnaKg && tezaNow - planTeden.ciljnaKg > 4)
    alarmi.push({ nivo: 'opozorilo', kat: 'Telo', naziv: `Teža ${tezaNow}kg — ${(tezaNow - planTeden.ciljnaKg).toFixed(1)}kg nad ciljem za T${currentTeden} (${planTeden.ciljnaKg}kg)`, fix: 'Zmanjšaj kalorije na počivalne dni (ne na teke).' })

  // ── KORAKI ───────────────────────────────────────────────
  const koraki7 = avg(metrike.slice(0, 7), 'koraki')
  if (koraki7 !== null && koraki7 < 5000)
    alarmi.push({ nivo: 'opozorilo', kat: 'Telo', naziv: `Dnevni koraki ${Math.round(koraki7)} (7d povp.) — premalo aktivnosti`, fix: 'Vsaj 8000 korakov/dan podpira regeneracijo, metabolizem in razpoloženje.' })
  ...
  if (z7.length >= 3) {
    ...
    // Neto energijski deficit (dejanske porabljene, ne vs CILJI)
    if (z7.length >= 4) {
      ...
      if (netBalance < -900)
        alarmi.push({ nivo: 'kritično', kat: 'Prehrana', naziv: `Neto deficit ${Math.round(netBalance)} kcal/dan (7d) — prevelik`, fix: 'Chronično podhranjenost razgrajuje mišice in znižuje HRV.' })
      else if (netBalance < -600)
        alarmi.push({ nivo: 'opozorilo', kat: 'Prehrana', naziv: `Neto deficit ${Math.round(netBalance)} kcal/dan (7d)`, fix: 'Deficit >600 kcal pri intenzivnem treningu upočasni adaptacijo in regeneracijo.' })
    }
  }
